...
class Flow_Control():
    ...
    def __init__(self):
        ...
        # Define search terms for columns
    ...
    def all_calls(self):
        ...
        with tab1:
            ...
            # Upload data
            # One upload is for the PAM data, the other is for the output file
            uploader = Load_Data()
            ...
            if data_df and output_file:
                ...
                # Find the max 
                max_finder = Find_Max()
                # Get the desired PAM values from each column in data_df-either max or from PAR = 1076
                ...
                # Not a critical feature so turn off
                # max_test = max_finder.max_check(data_df, max_dict)
                
                # Call export class
                exporter = Export()
                
                # Add the selected PAM values to the correct rows in output_df by genotype
                final_results_dict = exporter.arrange_by_genotype(max_dict)
                final_results_df = exporter.add_to_output(final_results_dict, output_df)
                # st.write(final_results_df)
                
                # Download the new output_df
...
class Load_Data():
    ...
    def import_data(data_files):
        ...
        """
        Import .csv PAM data files into pandas DataFrames.
        ...
        for file in data_files:
            ...
            if file.name.endswith(".csv"):
                ...
                if not success:
                    st.error(
                        f"❌ Unable to read CSV file {file.name}. "
    ...
    def upload_output(output_file):
        """Imports the .csv output file uploaded and returns a pandas df."""
...
class Find_Max():
    ...
    def get_max_par_values(self, data_dict, search_columns_A, search_columns_B):
        ...
        """Find the max value in the columns Y(II)and ETR.
           Finds the value in row PAR = 1076 for Y(NPQ).
        """
        ...
        if data_dict:
            ...
            for filename, df in data_dict.items():
                ...
                # Sub dict for filename as key
                max_results_dict[filename] = {}
                
                # Loop over the col names from list A
                for target in search_columns_A:
                    
                    # Find column by matching substring
                ...
                # Do the same for Y(NPQ) but find the value in PAR = 1076 row instead of max
    ...
    def max_check(self, data_dict, max_results_dict):
        """
        Checks to see if the max values found for Y(II) and Y(NPQ) are from the same row.
        Workflow:
            1. Take the first Y(II) or Y(NPQ) max value from max_results_dict
            2. Search the raw dataframe in data_dict for rows whose first column matches that value
            3. Pull that row
            4. Compare the whole row to the max values in max_results_dict
        ...
        for check in check_list:
            ...
            for filename, df in data_dict.items():
                ...
                # Get the first max value-accomplished using next-from the max_results_dict
                # and then move on. Otherwise return None.
                ...
                # Goes down the column and labels each element as true or false to find the element from
                # the dataframe that matches the target value from the max_results_dict
                row_mask = np.isclose(first_col, target_value)
                
                # Returns the entire row where there was a match in row_mask 
                ...
                # Verify the max values retrieved in max_results_dict came from a single row
                # Get max values in a list
                calc_max_vals = [value for key, value in max_results_dict[filename].items() if check in key]
                # st.write("calc vals: ", calc_max_vals)
                
                # Compare to the row retrieved from the dataframe
...
class Export():
    """
    Finds the genotype in the output_file and matches with the correct Y(II), ETR, and Y(NPQ) values for that genotype.
    """
    ...
    def arrange_by_genotype(self, max_dict):
        """
        Parse genotype and tank from PAM column names and organize results
        by GenotypeID (for example, F95_C1 or 12-45B_C1).
        """
        ...
        # Start a dict for results
        final_results_dict = defaultdict(dict)
        
        # Define search substrings to find columns in the output_file
        search_substrings = ["Y(II)", "Y(NPQ)", "ETR"]
        
        # Loop through results dictionaries and match values with GenotypeID
        for filename, results_dict in max_dict.items():
            ...
            for colname, val in results_dict.items():
                ...
                substring = next(
                                 (term for term in search_substrings if colname.startswith(term)),
                                 None
                                )
                ...
                # Remove the PAM metric and channel number, but preserve dashes
                prefix_pattern = rf"^{re.escape(substring)}\d+"
                remainder = re.sub(prefix_pattern, "", colname, count=1)
                
                # PAM headers may begin the genotype with either _ or -
                leading_separator = remainder[:1]
                remainder = remainder.lstrip("_-")
                parse = remainder.split("_")
                
                if len(parse) < 2:
                    with col1:
                        st.warning(f"Unexpected column format: {colname}")
                    continue
                
                tank = parse[-1]
                genotype_parts = parse[:-1]
                
                # A leading dash can split a dashed genotype across PAM fields
                if leading_separator == "-" and len(genotype_parts) > 1:
                    genotype = "-".join(genotype_parts)
                else:
                    genotype = "_".join(genotype_parts)
                
                genotype_id = f"{genotype}_{tank}"
                
                # Warn if the same metric/GenotypeID appears in multiple files
                if substring in final_results_dict[genotype_id]:
                    prev_file = final_results_dict[genotype_id].get("source", "unknown")
                    
                    with col1:
                        st.warning(
                                    f"Duplicate entry detected for GenotypeID "
                                    f"'{genotype_id}' and metric '{substring}'. "
                                    f"First seen in '{prev_file}', encountered again "
                                    f"in '{filename}'. Overwriting previous value."
                                  )
                
                final_results_dict[genotype_id][substring] = val
                final_results_dict[genotype_id]["source"] = filename
        
        return final_results_dict
    
    def add_to_output(self, final_results_dict, output_df):
        """
        Add newly calculated PAM values to matching GenotypeID rows while
        preserving values already present for all other genotypes and tanks.
        """
        ...
        # Check for missing columns in the output file
        ...
        output_df["GenotypeID"] = (output_df["GenotypeID"].astype(str).str.strip())
        
        output_columns = {
                          "Y(II)": "Y(II) max initial",
                          "Y(NPQ)": "Y(NPQ) max initial",
                          "ETR": "ETR max initial"
                         }
        
        # Update only GenotypeIDs represented in the uploaded PAM data
        for genotype_id, results in final_results_dict.items():
            row_mask = output_df["GenotypeID"] == genotype_id
            
            if not row_mask.any():
                with col1:
                    st.warning(
                               f"GenotypeID '{genotype_id}' was not found in the "
                               "output file."
                              )
                continue
            
            for metric, output_col in output_columns.items():
                value = results.get(metric)
                
                # Do not erase an existing value if the new PAM value is missing
                if value is None or pd.isna(value):
                    continue
                
                output_df.loc[row_mask, output_col] = value
