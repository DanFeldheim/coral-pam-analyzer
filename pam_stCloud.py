...
class Flow_Control():
    ...
    def __init__(self):
        ...
        self.search_columns = [
                               ...
                               'Y(NPQ)1', 'Y(NPQ)2', 'Y(NPQ)3', 'Y(NPQ)4', 'Y(NPQ)5',
    ...
    def all_calls(self):
        ...
        if import_data is not None:
            ...
            # Find the max 
            max_finder = Find_Max()
            max_dict = max_finder.max(import_data, self.search_columns)
            
            max_test = max_finder.max_check(import_data, max_dict)
...
class Find_Max():
    ...
    def max(self, data_dict, search_columns):
        ...
        if data_dict:
            ...
            for filename, df in data_dict.items():
                ...
                for target in search_columns:
                    matching_cols = [c for c in df.columns if c.startswith(target)]
    
                    if matching_cols:
                        max_results_dict[filename][target] = df[matching_cols[0]].max()
...
class Export():
    ...
    def download_max_values(self, results_dict):
        ...
        # Rename columns 
        df.rename(columns={
                          'Y(II)1':'Y(II)1_max_initial', 
                          'Y(II)2':'Y(II)2_max_initial', 
                          'Y(II)3':'Y(II)3_max_initial', 
                          'Y(II)4':'Y(II)4_max_initial', 
                          'Y(II)5':'Y(II)5_max_initial',
                          'Y(NPQ)1':'Y(NPQ)1_max_initial', 
                          'Y(NPQ)2':'Y(NPQ)2_max_initial', 
                          'Y(NPQ)3':'Y(NPQ)3_max_initial', 
                          'Y(NPQ)4':'Y(NPQ)4_max_initial', 
                          'Y(NPQ)5':'Y(NPQ)5_max_initial',
                          'ETR1':'ETR1_max_initial', 
                          'ETR2':'ETR2_max_initial', 
                          'ETR3':'ETR3_max_initial', 
                          'ETR4':'ETR4_max_initial', 
                          'ETR5':'ETR5_max_initial',
                          }, inplace=True)
        ...
        st.dataframe(df, hide_index=True, use_container_width=True, width=750)
